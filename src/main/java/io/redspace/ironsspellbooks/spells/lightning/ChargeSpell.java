...
public class ChargeSpell extends AbstractSpell {
    ...
    public void onCast(Level level, int spellLevel, LivingEntity entity, CastSource castSource, MagicData playerMagicData) {
        ...
        entity.addEffect(new MobEffectInstance(MobEffectRegistry.CHARGED.get(), (int) ((getSpellPower(spellLevel, entity) * 20)/4), spellLevel - 1, false, false, true));

        super.onCast(level, spellLevel, entity, castSource, playerMagicData);
    }

    private float getPercentAttackDamage(int spellLevel, LivingEntity entity) {
        return (spellLevel * ChargeEffect.ATTACK_DAMAGE_PER_LEVEL * 100) /4;
    }

    private float getPercentSpeed(int spellLevel, LivingEntity entity) {
        return (spellLevel * ChargeEffect.SPEED_PER_LEVEL * 100) /4;
    }

    private float getPercentSpellPower(int spellLevel, LivingEntity entity) {
        return (spellLevel * ChargeEffect.SPELL_POWER_PER_LEVEL * 100) /4;
    }
